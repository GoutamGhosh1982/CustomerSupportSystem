...
builder.Services.AddDbContext<ResponseDbContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("ResponseDb")));
...
builder.Services.AddScoped<IResponseService, ResponseAppService>();
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Response Service", Version = "v1" });
...
{
    var db = scope.ServiceProvider.GetRequiredService<ResponseDbContext>();
    db.Database.Migrate();
}
